# ----------------------------------------------------------------------------------------------------
# Flag-transitive 2-designs 
# Group (autSubgroup): A8
# ----------------------------------------------------------------------------------------------------
# Remark: all designs

# 1. Number of designs: 
# ---------------------

# ----------------------------------------------------
#                    Symmetric  Non-symmetric  Total  
# ----------------------------------------------------
# Point-primitive    3          18             21     
# Point-imprimitive  0          0              0      
#                                                     
# Block-primitive    3          5              8      
# Block-imprimitive  0          13             13     
# ----------------------------------------------------
# Total              3          18             21     
# ----------------------------------------------------

# 2. Summary: 
# -----------

# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Nr  v   b    r    k   λ   G   Gα                   GB                   Aut(D)  rk(G)  rk(Aut(D))  nr(Gα)  nr(GB) point-primitive  block-primitive  complement  symmetric  comments                                      
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 1   8   28   21   6   15  A8  A7                   S6                   S8      2      2           136     133    true             true                                                                                  
# 2   8   56   21   3   6   A8  A7                   3:S5                 S8      2      2           136     131    true             true             3                                                                    
# 3   8   56   35   5   20  A8  A7                   3:S5                 S8      2      2           136     131    true             true             2                                                                    
# 4   8   70   35   4   15  A8  A7                   (A4xA4):2            S8      2      2           136     129    true             false            4                                                                    
# 5   15  15   7    7   3   A8  2^3:PSL(3,2)         2^3:PSL(3,2)         A8      2      2           134     135    true             true             6           true       PG(3,2) or Hadamard parameters                
# 6   15  15   8    8   4   A8  2^3:PSL(3,2)         2^3:PSL(3,2)         A8      2      2           134     135    true             true             5           true       complement of PG(3,2) or Hadamard parameters  
# 7   15  35   7    3   1   A8  2^3:PSL(3,2)         ((A4xA4):2):2        A8      2      2           134     132    true             true             8                                                                    
# 8   15  35   28   12  22  A8  2^3:PSL(3,2)         ((A4xA4):2):2        A8      2      2           134     132    true             true             7                                                                    
# 9   15  105  28   4   6   A8  2^3:PSL(3,2)         (((2^4):2):3):2      A8      2      2           134     126    true             false                                                                                 
# 10  15  105  42   6   15  A8  2^3:PSL(3,2)         (((2^3):(2^2)):3):2  A8      2      2           134     124    true             false                                                                                 
# 11  15  120  56   7   24  A8  2^3:PSL(3,2)         PSL(3,2)             A8      2      2           134     122    true             false                                                                                 
# 12  15  168  56   5   16  A8  2^3:PSL(3,2)         S5                   A8      2      2           134     116    true             false            13                                                                   
# 13  15  168  112  10  72  A8  2^3:PSL(3,2)         S5                   A8      2      2           134     116    true             false            12                                                                   
# 14  15  280  112  6   40  A8  2^3:PSL(3,2)         (S3xS3):2            A8      2      2           134     106    true             false            15                                                                   
# 15  15  280  168  9   96  A8  2^3:PSL(3,2)         (S3xS3):2            A8      2      2           134     106    true             false            14                                                                   
# 16  15  420  84   3   12  A8  2^3:PSL(3,2)         2xS4                 A8      2      2           134     96     true             false                                                                                 
# 17  15  420  168  6   60  A8  2^3:PSL(3,2)         2xS4                 A8      2      2           134     98     true             false                                                                                 
# 18  15  840  224  4   48  A8  2^3:PSL(3,2)         S4                   A8      2      2           134     70     true             false                                                                                 
# 19  35  35   18   18  9   A8  ((((2^4):3):2):3):2  ((A4xA4):2):2        S8      3      3           132     132    true             true                         true       complement of Hadamard parameters             
# 20  56  840  180  12  36  A8  A5:S3                S4                   A8      4      4           131     74     true             false                                                                                 
# 21  56  840  180  12  36  A8  A5:S3                S4                   A8      4      4           131     74     true             false                                                                                 
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# 3. Further information: 
# -----------------------

# Design: 1
# -------------------------------------------------------------
# Parameter set: [ 8, 28, 21, 6, 15 ]
# Complement:    [ 8, 28, 7, 2, 1 ]
# -------------------------------------------------------------
#                                      G           Aut(D)      
# -------------------------------------------------------------
# Structure                            A8          S8          
# Rank                                 2           2           
# 2-Homogeneous                        true        true        
# Point-stabiliser                     A7          S7          
# Block-stabiliser                     S6          2xS6        
# Orbit structure of point-stabiliser  1^{1}7^{1}  1^{1}7^{1}  
# Orbit structure of block-stabiliser  2^{1}6^{1}  2^{1}6^{1}  
# Point-transitive                     true        true        
# Block-transitive                     true        true        
# Flag-transitive                      true        true        
# Anti-flag-transitive                 true        true        
# Flag-regular                         false       false       
# Point-primitive                      true        true        
# Point-primitive type                 2           2           
# Block-primitive                      true        true        
# Block-primitive type                 2           2           
# -------------------------------------------------------------

# Design: 2
# -------------------------------------------------------------
# Parameter set: [ 8, 56, 21, 3, 6 ]
# Complement:    [ 8, 56, 35, 5, 20 ]
# -------------------------------------------------------------
#                                      G           Aut(D)      
# -------------------------------------------------------------
# Structure                            A8          S8          
# Rank                                 2           2           
# 2-Homogeneous                        true        true        
# Point-stabiliser                     A7          S7          
# Block-stabiliser                     3:S5        S5xS3       
# Orbit structure of point-stabiliser  1^{1}7^{1}  1^{1}7^{1}  
# Orbit structure of block-stabiliser  3^{1}5^{1}  3^{1}5^{1}  
# Point-transitive                     true        true        
# Block-transitive                     true        true        
# Flag-transitive                      true        true        
# Anti-flag-transitive                 true        true        
# Flag-regular                         false       false       
# Point-primitive                      true        true        
# Point-primitive type                 2           2           
# Block-primitive                      true        true        
# Block-primitive type                 2           2           
# -------------------------------------------------------------

# Design: 3
# -------------------------------------------------------------
# Parameter set: [ 8, 56, 35, 5, 20 ]
# Complement:    [ 8, 56, 21, 3, 6 ]
# -------------------------------------------------------------
#                                      G           Aut(D)      
# -------------------------------------------------------------
# Structure                            A8          S8          
# Rank                                 2           2           
# 2-Homogeneous                        true        true        
# Point-stabiliser                     A7          S7          
# Block-stabiliser                     3:S5        S5xS3       
# Orbit structure of point-stabiliser  1^{1}7^{1}  1^{1}7^{1}  
# Orbit structure of block-stabiliser  3^{1}5^{1}  3^{1}5^{1}  
# Point-transitive                     true        true        
# Block-transitive                     true        true        
# Flag-transitive                      true        true        
# Anti-flag-transitive                 true        true        
# Flag-regular                         false       false       
# Point-primitive                      true        true        
# Point-primitive type                 2           2           
# Block-primitive                      true        true        
# Block-primitive type                 2           2           
# -------------------------------------------------------------

# Design: 4
# -------------------------------------------------------------
# Parameter set: [ 8, 70, 35, 4, 15 ]
# Complement:    [ 8, 70, 35, 4, 15 ]
# -------------------------------------------------------------
#                                      G           Aut(D)      
# -------------------------------------------------------------
# Structure                            A8          S8          
# Rank                                 2           2           
# 2-Homogeneous                        true        true        
# Point-stabiliser                     A7          S7          
# Block-stabiliser                     (A4xA4):2   S4xS4       
# Orbit structure of point-stabiliser  1^{1}7^{1}  1^{1}7^{1}  
# Orbit structure of block-stabiliser  4^{2}       4^{2}       
# Point-transitive                     true        true        
# Block-transitive                     true        true        
# Flag-transitive                      true        true        
# Anti-flag-transitive                 true        true        
# Flag-regular                         false       false       
# Point-primitive                      true        true        
# Point-primitive type                 2           2           
# Block-primitive                      false       false       
# Block-primitive type                 0           0           
# -------------------------------------------------------------

# Design: 5
# -----------------------------------------------------------------
# Parameter set: [ 15, 15, 7, 7, 3 ]
# Complement:    [ 15, 15, 8, 8, 4 ]
# -----------------------------------------------------------------
#                                      G             Aut(D)        
# -----------------------------------------------------------------
# Structure                            A8            A8            
# Rank                                 2             2             
# 2-Homogeneous                        true          true          
# Point-stabiliser                     2^3:PSL(3,2)  2^3:PSL(3,2)  
# Block-stabiliser                     2^3:PSL(3,2)  2^3:PSL(3,2)  
# Orbit structure of point-stabiliser  1^{1}14^{1}   1^{1}14^{1}   
# Orbit structure of block-stabiliser  7^{1}8^{1}    7^{1}8^{1}    
# Point-transitive                     true          true          
# Block-transitive                     true          true          
# Flag-transitive                      true          true          
# Anti-flag-transitive                 true          true          
# Flag-regular                         false         false         
# Point-primitive                      true          true          
# Point-primitive type                 2             2             
# Block-primitive                      true          true          
# Block-primitive type                 2             2             
# -----------------------------------------------------------------

# Design: 6
# -----------------------------------------------------------------
# Parameter set: [ 15, 15, 8, 8, 4 ]
# Complement:    [ 15, 15, 7, 7, 3 ]
# -----------------------------------------------------------------
#                                      G             Aut(D)        
# -----------------------------------------------------------------
# Structure                            A8            A8            
# Rank                                 2             2             
# 2-Homogeneous                        true          true          
# Point-stabiliser                     2^3:PSL(3,2)  2^3:PSL(3,2)  
# Block-stabiliser                     2^3:PSL(3,2)  2^3:PSL(3,2)  
# Orbit structure of point-stabiliser  1^{1}14^{1}   1^{1}14^{1}   
# Orbit structure of block-stabiliser  7^{1}8^{1}    7^{1}8^{1}    
# Point-transitive                     true          true          
# Block-transitive                     true          true          
# Flag-transitive                      true          true          
# Anti-flag-transitive                 true          true          
# Flag-regular                         false         false         
# Point-primitive                      true          true          
# Point-primitive type                 2             2             
# Block-primitive                      true          true          
# Block-primitive type                 2             2             
# -----------------------------------------------------------------

# Design: 7
# -------------------------------------------------------------------
# Parameter set: [ 15, 35, 7, 3, 1 ]
# Complement:    [ 15, 35, 28, 12, 22 ]
# -------------------------------------------------------------------
#                                      G              Aut(D)         
# -------------------------------------------------------------------
# Structure                            A8             A8             
# Rank                                 2              2              
# 2-Homogeneous                        true           true           
# Point-stabiliser                     2^3:PSL(3,2)   2^3:PSL(3,2)   
# Block-stabiliser                     ((A4xA4):2):2  ((A4xA4):2):2  
# Orbit structure of point-stabiliser  1^{1}14^{1}    1^{1}14^{1}    
# Orbit structure of block-stabiliser  3^{1}12^{1}    3^{1}12^{1}    
# Point-transitive                     true           true           
# Block-transitive                     true           true           
# Flag-transitive                      true           true           
# Anti-flag-transitive                 true           true           
# Flag-regular                         false          false          
# Point-primitive                      true           true           
# Point-primitive type                 2              2              
# Block-primitive                      true           true           
# Block-primitive type                 2              2              
# -------------------------------------------------------------------

# Design: 8
# -------------------------------------------------------------------
# Parameter set: [ 15, 35, 28, 12, 22 ]
# Complement:    [ 15, 35, 7, 3, 1 ]
# -------------------------------------------------------------------
#                                      G              Aut(D)         
# -------------------------------------------------------------------
# Structure                            A8             A8             
# Rank                                 2              2              
# 2-Homogeneous                        true           true           
# Point-stabiliser                     2^3:PSL(3,2)   2^3:PSL(3,2)   
# Block-stabiliser                     ((A4xA4):2):2  ((A4xA4):2):2  
# Orbit structure of point-stabiliser  1^{1}14^{1}    1^{1}14^{1}    
# Orbit structure of block-stabiliser  3^{1}12^{1}    3^{1}12^{1}    
# Point-transitive                     true           true           
# Block-transitive                     true           true           
# Flag-transitive                      true           true           
# Anti-flag-transitive                 true           true           
# Flag-regular                         false          false          
# Point-primitive                      true           true           
# Point-primitive type                 2              2              
# Block-primitive                      true           true           
# Block-primitive type                 2              2              
# -------------------------------------------------------------------

# Design: 9
# -----------------------------------------------------------------------
# Parameter set: [ 15, 105, 28, 4, 6 ]
# Complement:    [ 15, 105, 77, 11, 55 ]
# -----------------------------------------------------------------------
#                                      G                Aut(D)           
# -----------------------------------------------------------------------
# Structure                            A8               A8               
# Rank                                 2                2                
# 2-Homogeneous                        true             true             
# Point-stabiliser                     2^3:PSL(3,2)     2^3:PSL(3,2)     
# Block-stabiliser                     (((2^4):3):2):2  (((2^4):2):3):2  
# Orbit structure of point-stabiliser  1^{1}14^{1}      1^{1}14^{1}      
# Orbit structure of block-stabiliser  3^{1}4^{1}8^{1}  3^{1}4^{1}8^{1}  
# Point-transitive                     true             true             
# Block-transitive                     true             true             
# Flag-transitive                      true             true             
# Anti-flag-transitive                 false            false            
# Flag-regular                         false            false            
# Point-primitive                      true             true             
# Point-primitive type                 2                2                
# Block-primitive                      false            false            
# Block-primitive type                 0                0                
# -----------------------------------------------------------------------

# Design: 10
# -------------------------------------------------------------------------------
# Parameter set: [ 15, 105, 42, 6, 15 ]
# Complement:    [ 15, 105, 63, 9, 36 ]
# -------------------------------------------------------------------------------
#                                      G                    Aut(D)               
# -------------------------------------------------------------------------------
# Structure                            A8                   A8                   
# Rank                                 2                    2                    
# 2-Homogeneous                        true                 true                 
# Point-stabiliser                     2^3:PSL(3,2)         2^3:PSL(3,2)         
# Block-stabiliser                     (((2^3):(2^2)):3):2  (((2^3):(2^2)):3):2  
# Orbit structure of point-stabiliser  1^{1}14^{1}          1^{1}14^{1}          
# Orbit structure of block-stabiliser  1^{1}6^{1}8^{1}      1^{1}6^{1}8^{1}      
# Point-transitive                     true                 true                 
# Block-transitive                     true                 true                 
# Flag-transitive                      true                 true                 
# Anti-flag-transitive                 false                false                
# Flag-regular                         false                false                
# Point-primitive                      true                 true                 
# Point-primitive type                 2                    2                    
# Block-primitive                      false                false                
# Block-primitive type                 0                    0                    
# -------------------------------------------------------------------------------

# Design: 11
# -----------------------------------------------------------------
# Parameter set: [ 15, 120, 56, 7, 24 ]
# Complement:    [ 15, 120, 64, 8, 32 ]
# -----------------------------------------------------------------
#                                      G             Aut(D)        
# -----------------------------------------------------------------
# Structure                            A8            A8            
# Rank                                 2             2             
# 2-Homogeneous                        true          true          
# Point-stabiliser                     2^3:PSL(3,2)  2^3:PSL(3,2)  
# Block-stabiliser                     PSL(3,2)      PSL(3,2)      
# Orbit structure of point-stabiliser  1^{1}14^{1}   1^{1}14^{1}   
# Orbit structure of block-stabiliser  1^{1}7^{2}    1^{1}7^{2}    
# Point-transitive                     true          true          
# Block-transitive                     true          true          
# Flag-transitive                      true          true          
# Anti-flag-transitive                 false         false         
# Flag-regular                         false         false         
# Point-primitive                      true          true          
# Point-primitive type                 2             2             
# Block-primitive                      false         false         
# Block-primitive type                 0             0             
# -----------------------------------------------------------------

# Design: 12
# -----------------------------------------------------------------
# Parameter set: [ 15, 168, 56, 5, 16 ]
# Complement:    [ 15, 168, 112, 10, 72 ]
# -----------------------------------------------------------------
#                                      G             Aut(D)        
# -----------------------------------------------------------------
# Structure                            A8            A8            
# Rank                                 2             2             
# 2-Homogeneous                        true          true          
# Point-stabiliser                     2^3:PSL(3,2)  2^3:PSL(3,2)  
# Block-stabiliser                     S5            S5            
# Orbit structure of point-stabiliser  1^{1}14^{1}   1^{1}14^{1}   
# Orbit structure of block-stabiliser  5^{1}10^{1}   5^{1}10^{1}   
# Point-transitive                     true          true          
# Block-transitive                     true          true          
# Flag-transitive                      true          true          
# Anti-flag-transitive                 true          true          
# Flag-regular                         false         false         
# Point-primitive                      true          true          
# Point-primitive type                 2             2             
# Block-primitive                      false         false         
# Block-primitive type                 0             0             
# -----------------------------------------------------------------

# Design: 13
# -----------------------------------------------------------------
# Parameter set: [ 15, 168, 112, 10, 72 ]
# Complement:    [ 15, 168, 56, 5, 16 ]
# -----------------------------------------------------------------
#                                      G             Aut(D)        
# -----------------------------------------------------------------
# Structure                            A8            A8            
# Rank                                 2             2             
# 2-Homogeneous                        true          true          
# Point-stabiliser                     2^3:PSL(3,2)  2^3:PSL(3,2)  
# Block-stabiliser                     S5            S5            
# Orbit structure of point-stabiliser  1^{1}14^{1}   1^{1}14^{1}   
# Orbit structure of block-stabiliser  5^{1}10^{1}   5^{1}10^{1}   
# Point-transitive                     true          true          
# Block-transitive                     true          true          
# Flag-transitive                      true          true          
# Anti-flag-transitive                 true          true          
# Flag-regular                         false         false         
# Point-primitive                      true          true          
# Point-primitive type                 2             2             
# Block-primitive                      false         false         
# Block-primitive type                 0             0             
# -----------------------------------------------------------------

# Design: 14
# -----------------------------------------------------------------
# Parameter set: [ 15, 280, 112, 6, 40 ]
# Complement:    [ 15, 280, 168, 9, 96 ]
# -----------------------------------------------------------------
#                                      G             Aut(D)        
# -----------------------------------------------------------------
# Structure                            A8            A8            
# Rank                                 2             2             
# 2-Homogeneous                        true          true          
# Point-stabiliser                     2^3:PSL(3,2)  2^3:PSL(3,2)  
# Block-stabiliser                     (S3xS3):2     (S3xS3):2     
# Orbit structure of point-stabiliser  1^{1}14^{1}   1^{1}14^{1}   
# Orbit structure of block-stabiliser  6^{1}9^{1}    6^{1}9^{1}    
# Point-transitive                     true          true          
# Block-transitive                     true          true          
# Flag-transitive                      true          true          
# Anti-flag-transitive                 true          true          
# Flag-regular                         false         false         
# Point-primitive                      true          true          
# Point-primitive type                 2             2             
# Block-primitive                      false         false         
# Block-primitive type                 0             0             
# -----------------------------------------------------------------

# Design: 15
# -----------------------------------------------------------------
# Parameter set: [ 15, 280, 168, 9, 96 ]
# Complement:    [ 15, 280, 112, 6, 40 ]
# -----------------------------------------------------------------
#                                      G             Aut(D)        
# -----------------------------------------------------------------
# Structure                            A8            A8            
# Rank                                 2             2             
# 2-Homogeneous                        true          true          
# Point-stabiliser                     2^3:PSL(3,2)  2^3:PSL(3,2)  
# Block-stabiliser                     (S3xS3):2     (S3xS3):2     
# Orbit structure of point-stabiliser  1^{1}14^{1}   1^{1}14^{1}   
# Orbit structure of block-stabiliser  6^{1}9^{1}    6^{1}9^{1}    
# Point-transitive                     true          true          
# Block-transitive                     true          true          
# Flag-transitive                      true          true          
# Anti-flag-transitive                 true          true          
# Flag-regular                         false         false         
# Point-primitive                      true          true          
# Point-primitive type                 2             2             
# Block-primitive                      false         false         
# Block-primitive type                 0             0             
# -----------------------------------------------------------------

# Design: 16
# -----------------------------------------------------------------------
# Parameter set: [ 15, 420, 84, 3, 12 ]
# Complement:    [ 15, 420, 336, 12, 264 ]
# -----------------------------------------------------------------------
#                                      G                Aut(D)           
# -----------------------------------------------------------------------
# Structure                            A8               A8               
# Rank                                 2                2                
# 2-Homogeneous                        true             true             
# Point-stabiliser                     2^3:PSL(3,2)     2^3:PSL(3,2)     
# Block-stabiliser                     2xS4             2xS4             
# Orbit structure of point-stabiliser  1^{1}14^{1}      1^{1}14^{1}      
# Orbit structure of block-stabiliser  1^{1}3^{2}8^{1}  1^{1}3^{2}8^{1}  
# Point-transitive                     true             true             
# Block-transitive                     true             true             
# Flag-transitive                      true             true             
# Anti-flag-transitive                 false            false            
# Flag-regular                         false            false            
# Point-primitive                      true             true             
# Point-primitive type                 2                2                
# Block-primitive                      false            false            
# Block-primitive type                 0                0                
# -----------------------------------------------------------------------

# Design: 17
# -----------------------------------------------------------------------
# Parameter set: [ 15, 420, 168, 6, 60 ]
# Complement:    [ 15, 420, 252, 9, 144 ]
# -----------------------------------------------------------------------
#                                      G                Aut(D)           
# -----------------------------------------------------------------------
# Structure                            A8               A8               
# Rank                                 2                2                
# 2-Homogeneous                        true             true             
# Point-stabiliser                     2^3:PSL(3,2)     2^3:PSL(3,2)     
# Block-stabiliser                     2xS4             2xS4             
# Orbit structure of point-stabiliser  1^{1}14^{1}      1^{1}14^{1}      
# Orbit structure of block-stabiliser  1^{1}2^{1}6^{2}  1^{1}2^{1}6^{2}  
# Point-transitive                     true             true             
# Block-transitive                     true             true             
# Flag-transitive                      true             true             
# Anti-flag-transitive                 false            false            
# Flag-regular                         false            false            
# Point-primitive                      true             true             
# Point-primitive type                 2                2                
# Block-primitive                      false            false            
# Block-primitive type                 0                0                
# -----------------------------------------------------------------------

# Design: 18
# -----------------------------------------------------------------------
# Parameter set: [ 15, 840, 224, 4, 48 ]
# Complement:    [ 15, 840, 616, 11, 440 ]
# -----------------------------------------------------------------------
#                                      G                Aut(D)           
# -----------------------------------------------------------------------
# Structure                            A8               A8               
# Rank                                 2                2                
# 2-Homogeneous                        true             true             
# Point-stabiliser                     2^3:PSL(3,2)     2^3:PSL(3,2)     
# Block-stabiliser                     S4               S4               
# Orbit structure of point-stabiliser  1^{1}14^{1}      1^{1}14^{1}      
# Orbit structure of block-stabiliser  1^{1}4^{2}6^{1}  1^{1}4^{2}6^{1}  
# Point-transitive                     true             true             
# Block-transitive                     true             true             
# Flag-transitive                      true             true             
# Anti-flag-transitive                 false            false            
# Flag-regular                         false            false            
# Point-primitive                      true             true             
# Point-primitive type                 2                2                
# Block-primitive                      false            false            
# Block-primitive type                 0                0                
# -----------------------------------------------------------------------

# Design: 19
# -----------------------------------------------------------------------------
# Parameter set: [ 35, 35, 18, 18, 9 ]
# Complement:    [ 35, 35, 17, 17, 8 ]
# -----------------------------------------------------------------------------
#                                      G                    Aut(D)             
# -----------------------------------------------------------------------------
# Structure                            A8                   S8                 
# Rank                                 3                    3                  
# 2-Homogeneous                        false                false              
# Point-stabiliser                     ((((2^4):3):2):3):2  (S4xS4):2          
# Block-stabiliser                     ((A4xA4):2):2        (S4xS4):2          
# Orbit structure of point-stabiliser  1^{1}16^{1}18^{1}    1^{1}16^{1}18^{1}  
# Orbit structure of block-stabiliser  1^{1}16^{1}18^{1}    1^{1}16^{1}18^{1}  
# Point-transitive                     true                 true               
# Block-transitive                     true                 true               
# Flag-transitive                      true                 true               
# Anti-flag-transitive                 false                false              
# Flag-regular                         false                false              
# Point-primitive                      true                 true               
# Point-primitive type                 2                    2                  
# Block-primitive                      true                 true               
# Block-primitive type                 2                    2                  
# -----------------------------------------------------------------------------

# Design: 20
# ---------------------------------------------------------------------------------------
# Parameter set: [ 56, 840, 180, 12, 36 ]
# Complement:    [ 56, 840, 660, 44, 516 ]
# ---------------------------------------------------------------------------------------
#                                      G                        Aut(D)                   
# ---------------------------------------------------------------------------------------
# Structure                            A8                       A8                       
# Rank                                 4                        4                        
# 2-Homogeneous                        false                    false                    
# Point-stabiliser                     3:S5                     3:S5                     
# Block-stabiliser                     S4                       S4                       
# Orbit structure of point-stabiliser  1^{1}10^{1}15^{1}30^{1}  1^{1}10^{1}15^{1}30^{1}  
# Orbit structure of block-stabiliser  4^{2}12^{4}              4^{2}12^{4}              
# Point-transitive                     true                     true                     
# Block-transitive                     true                     true                     
# Flag-transitive                      true                     true                     
# Anti-flag-transitive                 false                    false                    
# Flag-regular                         false                    false                    
# Point-primitive                      true                     true                     
# Point-primitive type                 2                        2                        
# Block-primitive                      false                    false                    
# Block-primitive type                 0                        0                        
# ---------------------------------------------------------------------------------------

# Design: 21
# ---------------------------------------------------------------------------------------
# Parameter set: [ 56, 840, 180, 12, 36 ]
# Complement:    [ 56, 840, 660, 44, 516 ]
# ---------------------------------------------------------------------------------------
#                                      G                        Aut(D)                   
# ---------------------------------------------------------------------------------------
# Structure                            A8                       A8                       
# Rank                                 4                        4                        
# 2-Homogeneous                        false                    false                    
# Point-stabiliser                     3:S5                     3:S5                     
# Block-stabiliser                     S4                       S4                       
# Orbit structure of point-stabiliser  1^{1}10^{1}15^{1}30^{1}  1^{1}10^{1}15^{1}30^{1}  
# Orbit structure of block-stabiliser  4^{2}12^{4}              4^{2}12^{4}              
# Point-transitive                     true                     true                     
# Block-transitive                     true                     true                     
# Flag-transitive                      true                     true                     
# Anti-flag-transitive                 false                    false                    
# Flag-regular                         false                    false                    
# Point-primitive                      true                     true                     
# Point-primitive type                 2                        2                        
# Block-primitive                      false                    false                    
# Block-primitive type                 0                        0                        
# ---------------------------------------------------------------------------------------

# 4. Designs: 
# -----------

lD := [ 
 rec(
  autGroup := SymmetricGroup( [ 1 .. 8 ] ),
  autSubgroup := AlternatingGroup( [ 1 .. 8 ] ),
  baseBlock := [ 3, 4, 5, 6, 7, 8 ],
  blockSizes := [ 6 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 8, 28, 21, 6, 15 ],
  r := 21,
  tSubsetStructure := rec(
      lambdas := [ 15 ],
      t := 2 ),
  v := 8 ), 
 rec(
  autGroup := SymmetricGroup( [ 1 .. 8 ] ),
  autSubgroup := AlternatingGroup( [ 1 .. 8 ] ),
  baseBlock := [ 3, 4, 7 ],
  blockSizes := [ 3 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 8, 56, 21, 3, 6 ],
  r := 21,
  tSubsetStructure := rec(
      lambdas := [ 6 ],
      t := 2 ),
  v := 8 ), 
 rec(
  autGroup := SymmetricGroup( [ 1 .. 8 ] ),
  autSubgroup := AlternatingGroup( [ 1 .. 8 ] ),
  baseBlock := [ 1, 2, 5, 6, 8 ],
  blockSizes := [ 5 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 8, 56, 35, 5, 20 ],
  r := 35,
  tSubsetStructure := rec(
      lambdas := [ 20 ],
      t := 2 ),
  v := 8 ), 
 rec(
  autGroup := SymmetricGroup( [ 1 .. 8 ] ),
  autSubgroup := AlternatingGroup( [ 1 .. 8 ] ),
  baseBlock := [ 1, 3, 6, 7 ],
  blockSizes := [ 4 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 8, 70, 35, 4, 15 ],
  r := 35,
  tSubsetStructure := rec(
      lambdas := [ 15 ],
      t := 2 ),
  v := 8 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 1, 5, 7, 8, 9, 11, 15 ],
  blockSizes := [ 7 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 15, 7, 7, 3 ],
  r := 7,
  tSubsetStructure := rec(
      lambdas := [ 3 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 2, 3, 4, 6, 10, 12, 13, 14 ],
  blockSizes := [ 8 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 15, 8, 8, 4 ],
  r := 8,
  tSubsetStructure := rec(
      lambdas := [ 4 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 6, 11, 14 ],
  blockSizes := [ 3 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 35, 7, 3, 1 ],
  r := 7,
  tSubsetStructure := rec(
      lambdas := [ 1 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 1, 2, 3, 4, 5, 7, 8, 9, 10, 12, 13, 15 ],
  blockSizes := [ 12 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 35, 28, 12, 22 ],
  r := 28,
  tSubsetStructure := rec(
      lambdas := [ 22 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 2, 6, 7, 11 ],
  blockSizes := [ 4 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 105, 28, 4, 6 ],
  r := 28,
  tSubsetStructure := rec(
      lambdas := [ 6 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 2, 6, 7, 11, 13, 14 ],
  blockSizes := [ 6 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 105, 42, 6, 15 ],
  r := 42,
  tSubsetStructure := rec(
      lambdas := [ 15 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 1, 3, 4, 5, 8, 9, 12 ],
  blockSizes := [ 7 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 120, 56, 7, 24 ],
  r := 56,
  tSubsetStructure := rec(
      lambdas := [ 24 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 6,13, 9,14, 5, 8)( 3,15,11,10, 7,12, 4), ( 1, 8,11)( 2,12, 5)( 3, 6, 9)( 4,10, 7)(13,14,15) ] ),
  baseBlock := [ 5, 11, 12, 14, 15 ],
  blockSizes := [ 5 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 168, 56, 5, 16 ],
  r := 56,
  tSubsetStructure := rec(
      lambdas := [ 16 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 1, 2, 3, 4, 5, 8, 12, 13, 14, 15 ],
  blockSizes := [ 10 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 168, 112, 10, 72 ],
  r := 112,
  tSubsetStructure := rec(
      lambdas := [ 72 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 1, 2, 3, 6, 8, 12 ],
  blockSizes := [ 6 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 280, 112, 6, 40 ],
  r := 112,
  tSubsetStructure := rec(
      lambdas := [ 40 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 4, 5, 7, 9, 10, 11, 13, 14, 15 ],
  blockSizes := [ 9 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 280, 168, 9, 96 ],
  r := 168,
  tSubsetStructure := rec(
      lambdas := [ 96 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 6, 11, 13 ],
  blockSizes := [ 3 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 420, 84, 3, 12 ],
  r := 84,
  tSubsetStructure := rec(
      lambdas := [ 12 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 1, 3, 5, 8, 9, 10 ],
  blockSizes := [ 6 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 420, 168, 6, 60 ],
  r := 168,
  tSubsetStructure := rec(
      lambdas := [ 60 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 7,12)( 8,13)(10,15), ( 2, 3)( 8,11)( 9,15)(12,14), ( 2, 4)( 3,13)( 6,12)(10,14), ( 4, 5)( 7,13)( 8,12)(11,14), ( 5, 6)( 7,10)( 9,14)(12,15), 
      ( 5, 9)( 6,14)( 7,15)(10,12), ( 5, 7)( 6,10)( 9,15)(12,14) ] ),
  autSubgroup := Group( [ ( 1, 8, 7, 6,15, 3,14)( 2,10, 5,12,11, 9,13), ( 1,11, 8)( 2, 5,12)( 3, 9, 6)( 4, 7,10)(13,15,14) ] ),
  baseBlock := [ 2, 4, 10, 12 ],
  blockSizes := [ 4 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 15, 840, 224, 4, 48 ],
  r := 224,
  tSubsetStructure := rec(
      lambdas := [ 48 ],
      t := 2 ),
  v := 15 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 4, 5)( 7, 9)(11,13)(14,15)(17,18)(21,23)(24,25)(27,28)(31,33), ( 2, 3)( 5, 6)( 7, 8)(11,12)(14,16)(18,19)(21,22)(24,26)(28,29)(31,32), 
      ( 3, 6)( 7,17)( 8,20)( 9,18)(10,19)(12,16)(21,24)(23,25)(27,31)(28,33)(29,34)(30,32), ( 3,10)( 4, 7)( 5, 9)(11,17)(13,18)(16,20)(22,30)(24,27)(25,28)(32,35), (11,24)(12,26)(13,25)(14,21)(15,23)(16,22)(17,27)
        (18,28)(19,29)(20,30) ] ),
  autSubgroup := Group( [ ( 1,15,23, 4,27, 2,29)( 3,14,19,25,33, 7,30)( 5,16,21,32, 9,11,22)( 6,28,34, 8,13,17,24)(10,12,18,20,26,31,35), 
      ( 1,23,15)( 2,21,14)( 3,22,16)( 4,25,13)( 5,24,11)( 6,26,12)( 7,28,18)( 8,29,19)( 9,27,17)(10,30,20) ] ),
  baseBlock := [ 4, 7, 8, 11, 12, 14, 16, 17, 20, 21, 22, 24, 26, 27, 30, 31, 32, 35 ],
  blockSizes := [ 18 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 35, 35, 18, 18, 9 ],
  r := 18,
  tSubsetStructure := rec(
      lambdas := [ 9 ],
      t := 2 ),
  v := 35 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 3,53)( 5,47)( 6,34)( 7,24)( 8,40)( 9,38)(10,12)(13,36)(14,28)(15,54)(16,41)(18,21)(19,25)(20,33)(23,39)(27,37)(31,56)(42,46)(43,48)(44,55)(45,50), 
      ( 2, 4)( 5,40)( 7,17)( 8,42)( 9,28)(10,31)(12,25)(13,26)(14,23)(15,32)(16,50)(19,56)(20,35)(21,30)(22,37)(29,48)(34,49)(38,39)(41,55)(44,45)(46,47)(51,53), ( 4, 7)( 6,16)( 8,48)( 9,54)(11,24)
        (12,44)(13,40)(15,38)(18,43)(19,34)(20,51)(21,36)(22,31)(23,41)(25,39)(27,28)(29,35)(30,33)(32,45)(42,53)(49,56)(50,52), ( 3,16)( 4,20)( 5,54)( 7,49)(11,33)(12,44)(13,28)(14,48)
        (15,47)(17,29)(18,40)(19,53)(22,24)(23,34)(25,46)(27,43)(30,31)(32,45)(36,37)(41,42)(50,52)(51,56), ( 3,46)( 4,11)( 6,39)( 7,24)( 8,21)(10,55)(12,44)(13,43)(14,37)(16,25)(18,40)(19,41)(20,33)
        (22,49)(23,34)(27,28)(30,51)(31,56)(32,52)(36,48)(42,53)(45,50) ] ),
  autSubgroup := Group( [ ( 1, 2, 4, 7,11,17,24)( 3, 5, 8,13,21,29,33)( 6,10,16, 9,15,23,31)(12,19,28,38,32,41,49)(14,22,25,34,44,50,54)(18,26,36,46,53,51,20)(27,37,39,45,52,55,56)(30,35,43,47,42,40,48), 
      ( 1, 3, 6)( 5, 9,10)( 7,12,20)( 8,14,15)(11,18,27)(13,22,30)(17,25,35)(21,26,37)(23,32,42)(24,33,43)(28,31,40)(29,39,46)(34,45,53)(38,47,48)(44,51,49) ] ),
  baseBlock := [ 2, 3, 18, 19, 20, 22, 27, 34, 36, 39, 48, 53 ],
  blockSizes := [ 12 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 56, 840, 180, 12, 36 ],
  r := 180,
  tSubsetStructure := rec(
      lambdas := [ 36 ],
      t := 2 ),
  v := 56 ), 
 rec(
  autGroup := Group( [ ( 1, 2)( 3,53)( 5,47)( 6,34)( 7,24)( 8,40)( 9,38)(10,12)(13,36)(14,28)(15,54)(16,41)(18,21)(19,25)(20,33)(23,39)(27,37)(31,56)(42,46)(43,48)(44,55)(45,50), 
      ( 2, 4)( 5,40)( 7,17)( 8,42)( 9,28)(10,31)(12,25)(13,26)(14,23)(15,32)(16,50)(19,56)(20,35)(21,30)(22,37)(29,48)(34,49)(38,39)(41,55)(44,45)(46,47)(51,53), ( 4, 7)( 6,16)( 8,48)( 9,54)(11,24)
        (12,44)(13,40)(15,38)(18,43)(19,34)(20,51)(21,36)(22,31)(23,41)(25,39)(27,28)(29,35)(30,33)(32,45)(42,53)(49,56)(50,52), ( 3,16)( 4,20)( 5,54)( 7,49)(11,33)(12,44)(13,28)(14,48)
        (15,47)(17,29)(18,40)(19,53)(22,24)(23,34)(25,46)(27,43)(30,31)(32,45)(36,37)(41,42)(50,52)(51,56), ( 3,46)( 4,11)( 6,39)( 7,24)( 8,21)(10,55)(12,44)(13,43)(14,37)(16,25)(18,40)(19,41)(20,33)
        (22,49)(23,34)(27,28)(30,51)(31,56)(32,52)(36,48)(42,53)(45,50) ] ),
  autSubgroup := Group( [ ( 1, 2, 4, 7,11,17,24)( 3, 5, 8,13,21,29,33)( 6,10,16, 9,15,23,31)(12,19,28,38,32,41,49)(14,22,25,34,44,50,54)(18,26,36,46,53,51,20)(27,37,39,45,52,55,56)(30,35,43,47,42,40,48), 
      ( 1, 3, 6)( 5, 9,10)( 7,12,20)( 8,14,15)(11,18,27)(13,22,30)(17,25,35)(21,26,37)(23,32,42)(24,33,43)(28,31,40)(29,39,46)(34,45,53)(38,47,48)(44,51,49) ] ),
  baseBlock := [ 1, 10, 12, 14, 16, 21, 25, 28, 33, 40, 49, 55 ],
  blockSizes := [ 12 ],
  isBinary := true,
  isBlockDesign := true,
  isSimple := true,
  parameters := [ 56, 840, 180, 12, 36 ],
  r := 180,
  tSubsetStructure := rec(
      lambdas := [ 36 ],
      t := 2 ),
  v := 56 )
]; 
for D in lD do D.blocks := Set( Orbit( D.autSubgroup , D.baseBlock , OnSets ) ); od; 

