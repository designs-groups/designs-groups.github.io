...
In each file, the information about the 2-designs D with automorphism group G is presented in four main sections. We may include a section named **Remark**. if there is no such section or the term "all designs" presented, it means that we have a complete classification of the designs with G as their block-transitive (flag-tranitive) automorphism group, otherwise, we present some information on the current status of classification.
